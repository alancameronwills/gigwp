...
- **Post metadata fields**: `dtstart`, `dtend`, `dtinfo`, `venue`, `recursday`, `recursweeks`, `recursfortnight`, `booklabel`, `bookinglink` (accepts an `https?://` URL or a `mailto:` link — the input fields on both the organizer submit form and the admin editor are `type="text"` with a `pattern` allowing either, not `type="url"`), `locallink`; plus moderation fields `gigio_approved` (`'0'` = pending, `'1'`/absent = approved, `'2'` = rejected), `gigio_organizer` (organizer row id), `gigio_organizer_email`
...
- **`[gigiau_submit]` page**: lets event organizers submit events without WordPress accounts. They sign in with a simple email + password (custom `{prefix}gigio_organizers` table, bcrypt hashes, NOT WordPress users), submit poster/title/date/venue/extra-info, and view/edit only their own events. On submit, `onSubmitEvent` (`gigio-submit.js`) gives immediate feedback via `setBusy(true)`: the Send button is disabled + greyed (CSS `.gigio-submit-btn:disabled`) and a `progress` activity cursor is shown page-wide until the request resolves.
